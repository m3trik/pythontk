...
class WeightsTest(unittest.TestCase):
    ...
    def test_generate_weights_count_is_stable_across_endpoints(self):
        ...
        for n in (1, 2, 3, 5):
            self.assertEqual(
                len(Weights.generate_weights(n, include_endpoints=False)), n
            )
            self.assertEqual(
                len(Weights.generate_weights(n, include_endpoints=True)), n
            )
    ...
    def test_generate_weights_strictly_inside_when_excluded(self):
        w = Weights.generate_weights(
            3, weight_range=(0.0, 1.0), include_endpoints=False
        )
    ...
    def test_generate_weights_single_with_endpoints(self):
        self.assertEqual(
            Weights.generate_weights(
                1, weight_range=(0.2, 0.8), include_endpoints=True
            ),
            [0.2],
        )
