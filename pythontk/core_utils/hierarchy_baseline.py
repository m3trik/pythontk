...
The record is a flat path set plus a hash.  Scoping is derived at compare time
from the roots being exported rather than stored, so nothing has to be keyed,
migrated or kept in step when an export is renamed, re-scoped or re-pointed:
...
class HierarchyBaseline:
    ...
    @classmethod
    def encode(cls, paths: Iterable[str]) -> Dict:
