...
@Deprecation.symbol(
    "subprocess directly, or a dedicated git library",
    remove_in="0.11.0",
    reason="It has no callers in any ecosystem package and no tests of its own.",
)
class Git:
    """A wrapper around git subprocess commands for a specific repository.

    **Deprecated** -- removed in 0.11.0. It has no callers anywhere in the
    ecosystem and no tests of its own, so it is published surface that only
    costs review attention and a name on the index. Shell out to ``git``
    ...
    def __init__(
        ...
        # The notice is on the class, not here: ``Deprecation.symbol`` wraps
        # ``__new__``, so a subclass warns too -- which a hand-written
        # ``__init__`` warn could not do, and a subclass is the one dependency
        # on a retired class that a grep for the name still finds.
