...
``.json`` (the keying recipes applied to shot objects — fades, audio clips, …).
...
This module is DCC-free (pure schema), so it imports and validates without any
...
folder under `user_config_root()/shots/manifest_behaviors/`; a user file
shadows a built-in of the same name. They are discovered automatically and
available everywhere a behavior name is accepted. JSON is used (stdlib,
