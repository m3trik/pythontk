...
- **`deliverer.py` — `PreviewDeliverer`** — the `Deliverer` strategy that builds the GLB from the produced FBX through `GlbPipeline` (`file_utils/mesh_convert/glb_pipeline.py` — the SAME downsize → `fbx_to_glb` → texture-pass chain the Scene Exporters run for their GLB output) and publishes it; `open_browser="auto"` opens a tab only when nothing is currently watching. `texture_format` selects WebP (default) or KTX2 delivery; the deliverer's own choices stop at that container and where its scratch files live.
