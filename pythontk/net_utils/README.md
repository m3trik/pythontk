...
- **`RemoteFile`** (`remote_file.py`) — the bytes behind an `http(s)` URL with one timeout, user agent and sign-in-wall check (`is_url`, `normalize`, `open`, `read_bytes`). Rewrites a Google Sheets share link to its CSV export endpoint, so a shared sheet is a data source with no API client; the shot-manifest CSV parser and the app installer's download both fetch through it.
...
The transport half of the [Live WebXR preview](../../docs/webxr_preview.md) pipeline (DCC → FBX → GLB → browser/headset). One subpackage, because the code ships with the page and scripts it serves:

- **`server.py` — `PreviewServer`** — threaded localhost static-file server with a live `manifest.json`; `publish()` bumps a version and the already-open page hot-swaps the model without a reload. Localhost is a design decision, not a limitation: `navigator.xr` requires a secure context and `http://localhost` is one by definition, so loopback buys a full `immersive-vr` session with no TLS for every PC-tethered headset. The bundled three.js page (`viewer.html`) and the optional viewer scripts (`scripts/*.js` — ES modules the page imports when the manifest names them; `SCRIPTS` / `add_script()`) sit beside it, which is how the viewer gains behaviour without being edited.
- **`deliverer.py` — `PreviewDeliverer`** — the `Deliverer` strategy that builds the GLB from the produced FBX through `GlbPipeline` (`file_utils/mesh_convert/glb_pipeline.py` — the SAME downsize → `fbx_to_glb` → texture-pass chain the Scene Exporters run for their GLB output) and publishes it; `open_browser="auto"` opens a tab only when nothing is currently watching. `glb_options` carries the Scene Exporter's GLB rows (`ExportProfile.GLB_ROWS`), resolved by the exporters' own `ExportRun.glb_texture_params` and `ExportRun.rendering`, so the preview runs the export's texture pass and publishes its lighting recipe; the deliverer's own choices stop at where its scratch files live.
- **`bridge.py` — `PreviewBridge`** — the `HandoffBridge` subclass supplying glTF-appropriate export defaults (embedded textures) plus the `push()` / `publish_file()` / `url` / `stop()` surface. It lives here — not mirrored per-DCC — because mayatk and blendertk cannot import each other, and anything written in both drifts in both.

`preview_server.py` is the previous single-module home, kept one release as a warning import alias.
