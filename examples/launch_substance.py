...
def launch_substance_painter(file_path=None, headless=False, enable_remote=True):
    ...
    proc = AppLauncher.launch(app_name, args=args, cwd=cwd, detached=True)
    ...
    if proc:
        ...
        print(
            "Failed to launch Substance Painter. Ensure it is installed and in your PATH."
        )


if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Launch Substance Painter via AppLauncher"
    )
    parser.add_argument("--file", "-f", help="Path to mesh or project file")
    parser.add_argument(
        "--headless", action="store_true", help="Launch in headless automation mode"
    )
    parser.add_argument(
        "--gui", action="store_true", help="Launch in standard GUI mode"
    )
    ...
    launch_substance_painter(
        file_path=args.file, headless=is_headless, enable_remote=True
    )
